...
const ROWS: { date: string; status: string; strike?: boolean; strikeFrame?: number }[] = [
  { date: "11 de septiembre", status: "agotado", strike: true, strikeFrame: 24 },
  { date: "18 de septiembre", status: "agotado", strike: true, strikeFrame: 50 },
  { date: "25 de septiembre", status: "12 lugares" },
];

// 0:53 - 0:57 (120f) — Direct callback to Scene 2's row grammar.
export const Scene13Escasez: React.FC = () => {
        ...
        <Card width={1000} padding={56}>
          {ROWS.map((row, i) => {
            const dim = row.strike
              ? interpolate(frame, [row.strikeFrame!, row.strikeFrame! + 16], [1, 0.3], {
                  extrapolateLeft: "clamp",
                  extrapolateRight: "clamp",
                })
              : 1;
            const strikeWidth = row.strike
              ? interpolate(frame, [row.strikeFrame!, row.strikeFrame! + 14], [0, 100], {
                  extrapolateLeft: "clamp",
                  extrapolateRight: "clamp",
                  easing: Easing.bezier(0.16, 1, 0.3, 1),
                })
