...
const totalAt = (frame: number) => {
  const t1 = interpolate(frame, [CLICK_FRAMES[0], CLICK_FRAMES[0] + 6], [0, 3], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const t2 = interpolate(frame, [CLICK_FRAMES[1], CLICK_FRAMES[1] + 6], [0, 3], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const t3 = interpolate(frame, [CLICK_FRAMES[2], CLICK_FRAMES[2] + 6], [0, 4], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
...
const CheckCircle: React.FC<{ checked: boolean; checkFrame: number }> = ({ checked, checkFrame }) => (
  <svg width={36} height={36} viewBox="0 0 36 36" style={{ flexShrink: 0 }}>
    ...
    {checked ? <StrokePath d="M 10 18 L 15 23 L 26 11" color={COLORS.carbon} strokeWidth={3} startFrame={checkFrame} durationInFrames={6} /> : null}
  </svg>
);
...
const TaskRow: React.FC<{ label: string; hours: number; selectFrame: number }> = ({ label, hours, selectFrame }) => {
  ...
  const barOpacity = Number.isFinite(selectFrame)
    ? interpolate(frame, [selectFrame, selectFrame + 6], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })
    : 0;
...
const Cursor: React.FC = () => {
  ...
  const waypoints = [
    { x: 1000, y: 60, at: 0 },
    { x: 286, y: rowCenterY(0) - 4, at: 30 },
    { x: 286, y: rowCenterY(1) - 4, at: 54 },
    { x: 286, y: rowCenterY(2) - 4, at: 78 },
    { x: 286, y: rowCenterY(2) - 4, at: 100 },
  ];
  let x = waypoints[0].x;
  let y = waypoints[0].y;
  ...
  return (
    <>
      ...
      {CLICK_FRAMES.map((clickFrame, i) => {
        ...
        const scale = interpolate(frame, [clickFrame, clickFrame + 12], [0, 2.2], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.out(Easing.cubic) });
        const opacity = interpolate(frame, [clickFrame, clickFrame + 3, clickFrame + 12], [0, 0.7, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
        return (
          <div
            key={i}
            style={{
...
// Phase A: 0-100 — approach + three selections.
const PhaseCalculator: React.FC = () => {
  const frame = useCurrentFrame();
  const headlineOpacity = interpolate(frame, [0, 6, 22, 30], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const exitOpacity = interpolate(frame, [90, 100], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const exitScale = interpolate(frame, [90, 100], [1, 0.94], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
...
// Phase B: 100-130 — the total, quick zoom.
const PhaseTotal: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.marfil, justifyContent: "center", alignItems: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity, scale: `${scale}` }}>
        <div style={{ fontFamily, fontSize: 150, fontWeight: 700, color: COLORS.durazno, lineHeight: 1 }}>10</div>
        <div style={{ fontFamily, fontSize: 36, fontWeight: 500, color: COLORS.carbon }}>horas a la semana.</div>
        <div style={{ fontFamily, fontSize: 24, fontWeight: 400, color: COLORS.petroleo }}>En tareas repetitivas.</div>
      </div>
    </AbsoluteFill>
  );
};

// Phase C: 130-172 — the concrete payoff.
const PhaseTransform: React.FC = () => {
  const frame = useCurrentFrame();
  const captionOpacity = interpolate(frame, [0, 8], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const leftOpacity = interpolate(frame, [8, 16], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const lineDraw = interpolate(frame, [18, 28], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const rightOpacity = interpolate(frame, [22, 30], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const rightScale = interpolate(frame, [22, 30], [0.92, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
    easing: Easing.out(Easing.cubic),
    output: "perceptual-scale",
...
export const CalculatorGif: React.FC = () => {
  return (
    <AbsoluteFill>
      <Sequence name="Calculator" durationInFrames={100} layout="none">
        <PhaseCalculator />
      </Sequence>
      <Sequence name="Total" from={100} durationInFrames={30} layout="none">
        <PhaseTotal />
      </Sequence>
